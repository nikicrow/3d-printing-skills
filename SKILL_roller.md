...
- Outputs are auto-filed next to the script, regardless of the working
  directory it is invoked from: previews go to `previews\rollers\` and STLs to
  `printable_files\rollers\` (the folders are created automatically).
...
- Preview: `previews/rollers/preview_<name>_<theme>.png` (name lowercased, spaces -> `_`)
- STL: `printable_files/rollers/roller_<name>_<theme>.stl`
